...
client for Implementing support for json messages. • ChatConnector reads user input and creates an object of ChatMessage type.
• ChatHistory when new message arrives, it is added to chatHistory. ChatHistory calls all of its observers. 
• ChatConnector instances outputs the messages to clients. • User singleton has methods for adding, removing users, 
checking if the user exist and toString() that returns  • Also Command list is added to the application. 
